...
bool Fuzzcoin::runOnBasicBlock(BasicBlock &BB, Module &M) {
    ...
    block_uid++;
    key = (int8_t)(block_uid & 0xff);
    g_hash = crc64(key, g_hash.edx, g_hash.eax);
    key = (int8_t)(block_uid>>8 & 0xff);
    g_hash = crc64(key, g_hash.edx, g_hash.eax);
    key = (int8_t)(block_uid>>16 & 0xff);
    g_hash = crc64(key, g_hash.edx, g_hash.eax);
    key = (int8_t)(block_uid>>24 & 0xff);
    g_hash = crc64(key, g_hash.edx, g_hash.eax);
    int32_t bb_hash = (g_hash.eax ^ g_hash.edx);
    ...
    Instruction* newInst = CallInst::Create(myFunc, arguments, "", InsertPos);
